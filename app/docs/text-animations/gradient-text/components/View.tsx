...
export default function View() {
  ...
  const [text, setText] = useState("Add a splash of color!");
    ...
    <div className="space-y-8 p-4">
      <div className="relative min-h-[25rem] bg-gray-900/30 rounded-lg p-8 border-2 border-purple-300 mt-8 flex items-center justify-center">
        <button
          ...
          aria-label="Re-initialize Gradient effect"
        >
          <svg
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm text-gray-300">
              ...
              <span className="text-purple-300 font-medium">{animationSpeed}s</span>
            </div>
            <input
              type="range"
              min="1"
            ...
            />
            <div className="space-y-2">
            <span className="block text-sm font-medium text-gray-300">Show Border</span>
            ...
            <div className="space-y-2">
              {colors.map((color, index) => (
                <div key={index} className="flex items-center gap-3">
                  <span className="text-sm text-gray-400 w-16">Color {index + 1}:</span>
                  <input
                    type="color"
                    value={color}
                    onChange={event => {
                    ...
                    className="h-8 w-8 rounded-md border border-gray-600 bg-gray-800 p-1"
                  />
                  <input
                    type="text"
                    value={color}
                    ...
                    className="flex-1 px-3 py-1 bg-gray-800 border border-gray-600 rounded-md text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent"
                  />
                </div>
              ))}
            </div>

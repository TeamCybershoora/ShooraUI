...
import { ScrollVelocity } from "@cybershoora/shoora-ui"

export default function View() {
  const [key, setKey] = useState(0);
  const [texts, setTexts] = useState(['Shoora UI', 'Scroll Down']);
  const [velocity, setVelocity] = useState(1);
  const [className, setClassName] = useState("custom-scroll-text");
    ...
    <div className="space-y-8 p-4">
      <div className="relative min-h-[25rem] bg-gray-900/30 rounded-lg p-8 border-2 border-purple-300 mt-8 flex items-center justify-center">
        <button
          ...
          aria-label="Re-initialize Scroll Velocity"
        >
          <svg
      ...
      <div>
        <h2 className="text-xl font-semibold text-white mb-4 flex items-center">
          ...
          Customize Scroll Velocity
        </h2>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2 md:col-span-2">
            <label className="block text-sm font-medium text-gray-300">Texts Array</label>
            <textarea
              value={texts.join('\n')}
              onChange={event => setTexts(event.target.value.split('\n').filter(text => text.trim()))}
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm text-gray-300">
              <span>Velocity</span>
              <span className="text-purple-300 font-medium">{velocity}</span>
            </div>
            <input
              type="range"
              min="0.1"
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-gray-800 border border-gray-600 rounded-md text-white focus:ring-2 focus:ring-purple-500 focus:border-transparent"
              placeholder="custom-scroll-text"
            />
          </div>
        </div>
      </div>
